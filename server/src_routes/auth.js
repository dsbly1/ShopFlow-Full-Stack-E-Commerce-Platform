...
const { sendVerificationEmail } = require('../email_v');
...
router.get('/verify-email', async (req, res) => {
  ...
  try {
    ...
    const { sendVerificationSuccessEmail } = require('../email_v');
...
const authMiddleware = require('../src_middleware/auth');
...
router.post('/forgot-password', async (req, res) => {
  ...
  await db.query('UPDATE users SET reset_token=$1, reset_token_expires_at=$2 WHERE email=$3', [token, expires, email]);
  const { sendPasswordResetEmail } = require('../email_v');
...
router.post('/reset-password', async (req, res) => {
  ...
  const { rows } = await db.query(
    'SELECT * FROM users WHERE reset_token=$1 AND reset_token_expires_at > NOW()', [token]
  );
  if (!rows.length) return res.status(400).json({ error: 'Invalid or expired reset link' });
  const hash = await bcrypt.hash(password, 10);
  await db.query('UPDATE users SET password_hash=$1, reset_token=NULL, reset_token_expires_at=NULL WHERE id=$2', [hash, rows[0].id]);
